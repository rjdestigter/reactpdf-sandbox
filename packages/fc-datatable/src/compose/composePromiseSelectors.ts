...
// Types
import { Filters, Record, RecordSelector, Selectors, Sort, State } from '../types'
...
export default function composeSelectors<R extends Record>({
  ...
  const sortedRecords$: (state: any) => R[] = createSelector(
    ...
    (records: R[], sortBy: keyof R | void, sortDesc: boolean, groupedBy: State<R>['groupedBy']): R[] => {
      let list: R[]
      const groupes = Object.keys(groupedBy)

      if (sortBy) {
        //
        const sorted: R[] = [...records].sort((record1: R, record2: R): number => {
          const value1 = sort && sort[sortBy] ? sort[sortBy](record1) : record1[sortBy]
          const value2 = sort && sort[sortBy] ? sort[sortBy](record1) : record2[sortBy]
      ...
      if (groupes.length && list.length) {
        return recursiveGroupBy(list, groupes.filter(column => groupedBy[column]))
      }
